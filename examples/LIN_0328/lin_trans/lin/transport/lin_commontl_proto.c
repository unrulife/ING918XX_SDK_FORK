...
static void lin_tl_handler_rx_completed(l_ifc_handle iii)
{
    ...
    if (prot_user_config_ptr->function == (bool)LIN_SLAVE)
    {
        ...
        if ((LD_BROADCAST != (*lin_tl_pdu)[0]) && (LD_FUNCTIONAL_NAD != (*lin_tl_pdu)[0]))
        {
            if (*node_attr_ptr->configured_NAD_ptr != (*lin_tl_pdu)[0])
            {
                invalid_NAD = ((node_attr_ptr->initial_NAD != (*lin_tl_pdu)[0]) ||
                               (SERVICE_ASSIGN_NAD != (*lin_tl_pdu)[2]) ||
                               (PCI_SF != pci_type));
            }
            else
            {
                invalid_NAD = ((*node_attr_ptr->configured_NAD_ptr != node_attr_ptr->initial_NAD) &&
                               (SERVICE_ASSIGN_NAD == (*lin_tl_pdu)[2]) &&
                               (PCI_SF == pci_type));
            }
        }
