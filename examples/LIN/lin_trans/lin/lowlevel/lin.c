...
{
    DEV_ASSERT((l_u8)iii < LIN_NUM_OF_IFCS);
    dbg_printf("lin_lld_set_response(%d,%d)\r\n",iii,response_length);
    l_u32 lin_hardware_instance;
    status_t retVal = STATUS_ERROR;
    const l_u8 * buffer;
    ...
    if ((response_length <= 8U) && (response_length != 0U))
    {
        ...
        dbg_printf("buffer:%x,%x,%x,%x,%x,%x,%x,%x\r\n",
...
void lin_lld_timeout_service(l_ifc_handle iii)
{
    ...
    LIN_DRV_TimeoutService(lin_hardware_instance);//只处理收发状态下的超时
    ...
    {
        case LIN_NODE_STATE_IDLE://第一次上电跑这里
            if (!(prot_state_ptr->idle_timeout_cnt-- > 0U))
            {
                /* Callback to handle timeout */
                CALLBACK_HANDLER(iii, LIN_LLD_BUS_ACTIVITY_TIMEOUT, 0xFFU);//从设备跑
                            ...
                            void * linState)
{
    ...
    //temp_printf("--instance =%d\r\n",instance);
    
    //temp_printf("--iii =%d\r\n",iii);
    const lin_protocol_user_config_t * prot_user_config_ptr = &g_lin_protocol_user_cfg_array[iii];
    lin_protocol_state_t * prot_state_ptr = &g_lin_protocol_state_array[iii];
    //temp_printf("--num =%d\r\n",prot_user_config_ptr->number_of_configurable_frames);
    temp_printf("linCurrentState->currentEventId-%x\r\n",linCurrentState->currentEventId);
#if (SUPPORT_MASTER_MODE == 1U)
