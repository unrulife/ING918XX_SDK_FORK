                           ...
                           l_u8 id)
{
    ...
    //temp_printf("process_id=%d\r\n",id);
    
    frame_index = lin_get_frame_index(iii, id);
    temp_printf("frame_index=%d\r\n",frame_index);
    ...
    if (0xFFU != frame_index)
    {
        ...
        temp_printf("frm_type=%d\r\n",lin_frame_ptr->frm_type);
                         ...
                         l_u8 id)
{
    ...
    //temp_printf("size=%d\r\n",sizeof(LI0_lin_configuration_RAM));
    for (i = prot_user_config_ptr->number_of_configurable_frames; i > 0U; i--)
    {
      //  temp_printf("RAM_ptr[%d]=%02x\r\n",i,prot_user_config_ptr->list_identifiers_RAM_ptr[i]);
                         ...
                         l_u8 id)
{
    ...
    const lin_protocol_user_config_t * prot_user_config_ptr = &g_lin_protocol_user_cfg_array[iii];
    lin_protocol_state_t * prot_state_ptr = &g_lin_protocol_state_array[iii];
    //temp_printf("lin_make_uncd_frame\r\n");
#if (1U == SUPPORT_SLAVE_MODE)
                                   ...
                                   l_u8 id)
{
    ...
    frame_index = lin_get_frame_index(iii, id);
    lin_frame_ptr = &(prot_user_config_ptr->frame_tbl_ptr[frame_index]);
    temp_printf("frm_response=%d,id=%d\r\n",lin_frame_ptr->frm_response,id);
    if (LIN_RES_PUB == lin_frame_ptr->frm_response)
    {
        action = 2U;
        if (0x3CU == id)
        {
            ...
            {
                temp_printf("LIN_TL_CALLBACK_HANDLER\r\n");
