    ...
    if (master_data_ptr->frm_offset != 0xFFFFU)
    {
        ...
        if ((size <= 8U) && ((size + offset) <= LIN_FRAME_BUF_SIZE))
        {
            ...
            for (i = 0U; i < size; i++)
            {
                ...
                dbg_printf("=======%02x\r\n",g_lin_frame_data_buffer[offset + i]);
            }
        }
    ...
    if (prot_user_config_ptr->function == (bool)LIN_MASTER)
    {
        ...
        if (LIN_SCH_TBL_NULL != active_schedule_struct_ptr->sch_tbl_type)
        {
            ...
            if (0U == prot_state_ptr->next_transmit_tick)
            {
                ...
                //temp_printf("cur_id =%d,frame_index=%d\r\n",cur_id,frame_index);                
               ...
               // temp_printf(" %d %d \r\n", cur_id, frame_index);
                ...
                if (*current_entry >= active_schedule_struct_ptr->num_slots)//ÇÐ»»½ø¶È±í
                {
       ...
       // temp_printf("current_id2=%x\r\n",prot_state_ptr->current_id);
